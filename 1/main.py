...
class MyWidget(QMainWindow):
    ...
    def update_coffee(self):
        cur = self.con.cursor()
        result = cur.execute('''
        SELECT coffee.id, coffee.name, roastings.roasting, types.type, coffee.price, coffee.extent 
        ...
        self.coffee_table.resizeRowsToContents()
    ...
    def change_coffee(self):
        ...
        cur = self.con.cursor()
        # settings[2] = cur.execute(f'SELECT id FROM genres WHERE title = "{settings[2]}"').fetchone()[0]
        cur.execute(f'''
        UPDATE coffee SET name = ?, roasting = ?, type = ?, taste = ?, price = ?, extent = ?
...
class CoffeeDialog(QDialog):
    def __init__(self, *parameters):
        ...
        if parameters:
            self.title_line.setText(parameters[0])
            self.year_line.setText(parameters[1])
            self.roasting = parameters[2]
            self.genres_list.setCurrentText(self.roasting)
            self.duration_line.setText(parameters[3])
    ...
    def fill_types(self):
        ...
        self.roastings_list.addItems(types)

    def set_type(self, name):
        database = sqlite3.connect('coffee.sqlite')
        cursor = database.cursor()
        self.roasting = cursor.execute(f'''SELECT id FROM types WHERE type = "{name}"''').fetchone()[0]
    ...
    def exec_(self) -> list:
        ...
        if not all([self.name_line.text(), self.taste_line.text(), self.price_line.text(), self.extent_line.text()]):
            return []
        return [self.name_line.text(), self.type, self.roasting, self.taste_line.text(), self.price_line.text(),
